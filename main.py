...
def validate_draw_date(draw_dt, today_dt, expected_weekdays, max_age_days=7):
    ...
    if age < 0 or age > max_age_days:
        raise ScrapeError(f"Draw date out of range: {draw_dt.date()} (age={age}d)")
    if draw_dt.weekday() not in expected_weekdays:
        raise ScrapeError(f"Draw date weekday mismatch: {draw_dt.date()}")
...
def parse_649(text, today_dt):
    ...
    if classic_idx == -1:
        raise ScrapeError("649: CLASSIC DRAW not found")

    date_matches = list(re.finditer(DATE_PATTERN, text[:classic_idx]))
    if not date_matches:
        raise ScrapeError("649: no date header found")
    draw_date_str = date_matches[-1].group(1)
    draw_dt = parse_draw_date(draw_date_str)
    validate_draw_date(draw_dt, today_dt, expected_weekdays=[2, 5], max_age_days=7)
    ...
    if not bonus_match:
        raise ScrapeError("649: bonus number not found")
    ...
    if len(winning_numbers) != 6 or len(set(winning_numbers)) != 6:
        raise ScrapeError("649: malformed winning numbers")
...
def parse_max(text, today_dt):
    ...
    if not date_matches:
        raise ScrapeError("Max: no date header found")
    first_match = date_matches[0]
    draw_date_str = first_match.group(1)
    draw_dt = parse_draw_date(draw_date_str)
    validate_draw_date(draw_dt, today_dt, expected_weekdays=[1, 4], max_age_days=7)
    ...
    bonus_match = re.search(r"Bonus\s*(\d{1,2})", segment)
    if not bonus_match:
        raise ScrapeError("Max: bonus number not found")
    bonus_num = int(bonus_match.group(1))
    ...
    if len(winning_numbers) != 7 or len(set(winning_numbers)) != 7:
        raise ScrapeError("Max: malformed winning numbers")
...
def fetch_home_jackpots(text, today_dt):
    ...
    if gb_match:
        ...
        gb_millions, balls_remaining, gb_next_date_str = 20, 18, "next draw"
    ...
    if not max_match:
        max_simple = re.search(r"\$\s*(\d+)\s*Million.*?" + date_re, text)
        if max_simple:
            max_millions = int(max_simple.group(1))
            maxplus_count = max_millions
            max_next_date_str = max_simple.group(2)
        else:
            max_millions, maxplus_count, max_next_date_str = 30, 30, "next draw"
    else:
        max_millions = int(max_match.group(1))
        maxplus_count = int(max_match.group(2)) if max_match.group(2) else max_millions
...
def replace_element_html(html_str, element_id, new_inner_html):
    ...
    if count == 0:
        raise ValueError(f"index_template.html: id={element_id!r} not found")
    return new_html
...
def set_element_class(html_str, element_id, new_class_value):
    ...
    if count == 0:
        raise ValueError(f"index_template.html: id={element_id!r} class not found")
    return new_html
...
def main():
    ...
    try:
        ...
        print(f"[WARN] Lotto Max scrape failed: {e}", file=sys.stderr)

    try:
        l649_text = fetch_text(WCLC_649_URL)
        l649_result = parse_649(l649_text, today_dt)
    except Exception as e:
        print(f"[WARN] Lotto 6/49 scrape failed: {e}", file=sys.stderr)
    ...
    if max_result:
        try:
            ...
            print(f"[WARN] Max history parse failed: {e}", file=sys.stderr)
    if l649_result:
        try:
            ...
            print(f"[WARN] 649 history parse failed: {e}", file=sys.stderr)
    ...
    try:
        ...
        print(f"[WARN] WCLC ticker scrape failed: {e}", file=sys.stderr)
    ...
    if max_result:
        ...
        if home_jackpots:
            ...
            max_prov = (
                ...
                f"No jackpot winner (Rolled over). "
                f"Winning ticket location: see official WCLC prize breakdown — {WCLC_MAX_PRIZE_PAGE}"
            )
        else:
            max_jp = "$30 Million"
            max_prov = f"Draw confirmed for {max_draw_date}. Next jackpot: $30 Million. See {WCLC_MAX_PRIZE_PAGE}"
        ...
        max_win_nums, max_bonus, max_draw_date = [12, 18, 19, 35, 43, 49, 52], 26, "Tuesday, September 8, 2026"
        max_jp, max_prov = "$30 Million", f"Draw confirmed for Tuesday, September 8, 2026. See {WCLC_MAX_PRIZE_PAGE}"
    ...
    print(f"Build completed successfully for {today_date}.")
