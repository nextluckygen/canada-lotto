...
def find_previous_post(posts_list, game, before_date=None):
    """posts_list(현재 posts_index.json 내용)에서 같은 게임의 가장 최근 포스트를
    찾아 그 전체 JSON을 읽어 반환한다. before_date를 주면 그보다 이전 날짜인
    포스트만 후보로 삼는다 — 같은 날 재실행될 때 자기 자신을 '직전 포스트'로
    잘못 찾는 것을 방지하기 위함이다."""
    candidates = [
        p for p in posts_list
        if p.get("game") == game and (before_date is None or p.get("date", "") < before_date)
    ]
    if not candidates:
        return None
...
def main():
    ...
    if max_result:
        ...
        if home_jackpots:
            ...
            maxplus_count = home_jackpots.get("max_maxplus_count")
            maxplus_text = (
                f"{maxplus_count} x $100,000 MaxPlus prizes" if maxplus_count is not None
                else "MaxPlus prize count unavailable this run"
            )
            max_prov = (
                f"Draw confirmed for {max_draw_date}. Next jackpot ({home_jackpots['max_next_draw']}): "
                f"{max_jp} ({maxplus_text}). "
    ...
    if weekday in [3, 6] and l649_result:  # 목/일 자정, 6/49 검증 성공시에만 게시
        ...
        prev_post = find_previous_post(posts_list, "Lotto 6/49", today_date)
        previous_pick_result = None
        if prev_post:
